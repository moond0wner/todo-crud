...
func (t *Task) Validate() error {
	...
	if t.Completed {
		if t.CompletedAt == nil {
			return fmt.Errorf(
				"Invalid `CompletedAt': can`t be `nil` if `Completed` == `true`: %w",
				core_errors.ErrInvalidArgument,
			)
		}
		if t.CompletedAt.Before(t.CreatedAt) {
			return fmt.Errorf(
				"Invalid `CompletedAt`: can`t be before `CreatedAt`: %w",
				core_errors.ErrInvalidArgument,
			)
		}
	} else {
		if t.CompletedAt != nil {
			return fmt.Errorf(
				"Invalid `CompletedAt`: must be `nil` if `Completed` == `false`: %w",
				core_errors.ErrInvalidArgument,
			)
		}
	}
